...
class MY_Controller extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $queryRapat = $this->model->get_seleksi_array('register_rapat', ['tanggal' => date('Y-m-d')], '');
        #die(var_dump($queryRapat));
        ...
        if (in_array($this->session->userdata('role'), ['super', 'validator_kepeg_satker', 'admin_satker'])) {
            ...
            $query = $this->model->get_seleksi_array('peran', ['userid' => $this->session->userdata('userid')]);
            if ($query->num_rows() > 0) {
